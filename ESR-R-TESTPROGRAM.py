"""
이 프로그램은 ESR-R개발보드에 연결된 모든 하드웨어 컴포넌트의
...
class ComponentTest:
    ...
    def update_display(self):
        ...
        if time.ticks_diff(current_time, self.last_display_update) > 500:
            ...
            self.oled.text(page_text, 100, 56)
    ...
    def display_page1(self):
        ...
        for i, (name, status) in enumerate(status_items):
            ...
            # 각 항목을 10픽셀 간격으로 세로 배치
            self.oled.text(f"{name}: {status_text}", 0, 12 + i * 10)
    ...
    def display_page3(self):
        ...
        # ========== 사용법 안내 ==========
        self.oled.text("Press button to", 0, 42)
        self.oled.text("cycle pages", 0, 52)
    ...
    def display_page4(self):
        ...
        # ========== 진행 중인 데모 상태 표시 ==========
        self.oled.text("WS2812B: Rainbow", 0, 42)
